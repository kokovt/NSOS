...
void kernel_main(void) {
  ...
  drawString("Needy Streamer Overload\nCopyright (C) 2020-\n\nNGO BIOS Rev1.0\n\nMain Processor : raincandy\nMemory Testing: OK", 0, 0, color);
  drawString("Booting Windose20", 0, size.height - 10,color);

  drawString("NSOS - Copyright (C) 2026 Kokovt/Onyx", size.width - 320, size.height - 40, color);
  drawString("Based upon NSO", size.width - 320, size.height - 30, color);
  drawString("This project is not affiliated with WSS", size.width - 320, size.height - 20, color);
  drawString("Playground or Microsoft", size.width - 320, size.height - 10, color);
  while (1) {
  }
}